...
def _configure_chip(env, config_file, chip_file):
    REPO_ROOT_DIR = env["REPO_ROOT_DIR"]
    KNOWN_CHIPS_FILE = REPO_ROOT_DIR.File(chip_file)
    ...
    # FIXME: figure out which exception to catch here
        ...
        print("Expected an 'mcu' key in XXXXmcuConfig.yaml")
        raise e
    ...
    # FIXME: figure out which exception to catch here
        ...
        print(
            "Chip specified in XXXmcuConfig.yaml does not have a translation. See site_scons/chips.yaml"
        )
        raise e
    ...
    if missing_drivers:
        raise Exception(
            ...
            "included in " + config_file +". Missing drivers:\n"
            f"{missing_drivers}"
        )
